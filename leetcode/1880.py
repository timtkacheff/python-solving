...
class Solution:
    ...
    def isSumEqual(self, firstWord: str,
                   secondWord: str, targetWord: str):
        firstWordIntValues = int("".join([str(ord(x) - 97)
                                          for x in [*firstWord]]))

        secondWordIntValues = int("".join([str(ord(x) - 97)
                                          for x in [*secondWord]]))

        targetWordIntValues = int("".join([str(ord(x) - 97)
                                          for x in [*targetWord]]))

        return firstWordIntValues + secondWordIntValues == targetWordIntValues
