...
# of the constructed 2D array, the elements from indices n to 2 * n - 1 (inclusive) should form 
...
class Solution:
    ...
    def construct2DArray(self, original: List[int], m: int, n: int) -> List[List[int]]:
        ...
        for row in range(0, m):
            last_row_index = (row + 1) * n - 1

            if len(original) != n * m:
                break

            result.append(original[first_row_index : last_row_index + 1])
